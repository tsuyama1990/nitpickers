...
@app.cell
def imports_cell() -> tuple[object, object, object, object, object, object, object, object, object, object, object]:
    import asyncio
    ...
    return AsyncMock, GraphBuilder, IntegrationState, JulesClient, MagicMock, Path, SandboxRunner, ServiceContainer, asyncio, patch, settings
...
@app.cell
def scenario1_cell(IntegrationState: object, Path: object, asyncio: object, integration_graph: object, mo: object, patch: object, settings: object) -> tuple[bool, object, object, bool]:
    if hasattr(mo, "md"):
    ...
    async def run_scenario_1() -> tuple[object, bool, bool]:
        ...
        with patch.object(settings.paths, "workspace_root", repo_path), patch("os.getcwd", return_value=str(repo_path)): # type: ignore
            with patch("src.nodes.master_integrator.MasterIntegratorNodes.master_integrator_node") as mock_mi, \
                 patch("src.nodes.sandbox_evaluator.SandboxEvaluatorNodes.sandbox_evaluate_node") as mock_sandbox, \
                 patch("src.services.git_ops.GitManager.merge_pr"): # type: ignore
                ...
                try:
                    result = await integration_graph.ainvoke( # type: ignore
                        state,
                        config={"configurable": {"thread_id": "uat_clean_merge"}}
                    )
    ...
    if hasattr(mo, "ui"):
        mo.ui.table([ # type: ignore
            {"Step": "Merge PR", "Expected": "Success", "Actual": "Success (Mocked)"},
            {"Step": "Global Sandbox", "Expected": "Called & Passed", "Actual": f"Called: {sandbox_called_1}"},
            {"Step": "Master Integrator", "Expected": "Skipped", "Actual": f"Called: {mi_called_1}"},
        ])
    return mi_called_1, result_1, run_scenario_1, sandbox_called_1


@app.cell
def scenario2_cell(IntegrationState: object, Path: object, asyncio: object, integration_graph: object, mo: object, patch: object, settings: object) -> tuple[int, bool, object, object, bool]:
    if hasattr(mo, "md"):
    ...
    async def run_scenario_2() -> tuple[object, bool, bool, int]:
        ...
        with patch.object(settings.paths, "workspace_root", repo_path), patch("os.getcwd", return_value=str(repo_path)): # type: ignore
            with patch("src.nodes.master_integrator.MasterIntegratorNodes.master_integrator_node") as mock_mi, \
                 patch("src.nodes.sandbox_evaluator.SandboxEvaluatorNodes.sandbox_evaluate_node") as mock_sandbox, \
                 patch("src.services.git_ops.GitManager.merge_pr") as mock_merge: # type: ignore
                ...
                try:
                    result = await integration_graph.ainvoke( # type: ignore
                        state,
                        config={"configurable": {"thread_id": "uat_conflict"}}
                    )
                except Exception as e:
    ...
    if hasattr(mo, "ui"):
        mo.ui.table([ # type: ignore
            ...
            {"Step": "Master Integrator", "Expected": "Called", "Actual": f"Called: {mi_called_2}"},
            {"Step": "Global Sandbox", "Expected": "Called & Passed", "Actual": f"Called: {sandbox_called_2}"},
        ])
    return merge_calls_2, mi_called_2, result_2, run_scenario_2, sandbox_called_2


@app.cell
def scenario3_cell(IntegrationState: object, Path: object, asyncio: object, integration_graph: object, mo: object, patch: object, settings: object) -> tuple[bool, object, object, int]:
    if hasattr(mo, "md"):
        ...
        with patch.object(settings.paths, "workspace_root", repo_path), patch("os.getcwd", return_value=str(repo_path)): # type: ignore
            with patch("src.nodes.integration_fixer.IntegrationFixerNodes.integration_fixer_node") as mock_fixer, \
                ...
                try:
                    result = await integration_graph.ainvoke( # type: ignore
                        state,
                        config={"configurable": {"thread_id": "uat_semantic"}}
                    )
                except Exception as e:
    ...
    if hasattr(mo, "ui"):
        mo.ui.table([ # type: ignore
            {"Step": "Global Sandbox attempts", "Expected": "2", "Actual": f"{sandbox_calls_3}"},
            {"Step": "Integration Fixer", "Expected": "Called", "Actual": f"Called: {fixer_called_3}"},
        ])
