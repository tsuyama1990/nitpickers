...
class RefactorUsecase:
    ...
    def _add_safe_files(self, modified_files: set[str], duplicates: list[list[dict[str, Any]]], complex_funcs: list[dict[str, Any]]) -> None:
        for group in duplicates:
            for item in group:
                self._add_single_safe_file(item["file"], modified_files)
        for item in complex_funcs:
            self._add_single_safe_file(item["file"], modified_files)

    def _add_single_safe_file(self, filepath: str, modified_files: set[str]) -> None:
        import ast
        path = Path(filepath).resolve()
        if path.suffix != ".py":
            logger.warning(f"File {filepath} is not a Python file. Skipping.")
            return
        ...
        try:
            if path.stat().st_size > settings.ast_analyzer.max_file_size_bytes:
                logger.warning(f"File {filepath} exceeds size limit. Skipping.")
                return

            with path.open(encoding="utf-8") as f:
                content = f.read()

            suspicious_patterns = ["os.system", "subprocess", "eval("]
            if any(p in content for p in suspicious_patterns):
                logger.warning(f"File {filepath} contains suspicious pattern. Skipping.")
                return

            ast.parse(content, filename=str(path))
            modified_files.add(str(path))

        except (SyntaxError, OSError, UnicodeDecodeError) as parse_err:
            logger.warning(f"File {filepath} has invalid syntax or read error: {parse_err}. Skipping.")
    ...
    async def execute(self) -> GlobalRefactorResult:
        ...
        self._add_safe_files(modified_files, duplicates, complex_funcs)
        ...
        try:
            ...
            # Secure error logging. We only log the error type and a safe generic message
            # to prevent API keys or stack traces from spilling to log aggregators.
            logger.error(f"Global refactoring LLM session failed due to {type(e).__name__}.")
            return GlobalRefactorResult(
                refactorings_applied=False,
