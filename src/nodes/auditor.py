...
class AuditorNodes:
    ...
    async def auditor_node(self, state: CycleState) -> dict[str, Any]:
        ...
        if getattr(state, "uat_execution_state", None):
            uat_usecase = UATAuditorUseCase(LLMReviewer(), e2b_tools=self.e2b_tools)
            return dict(await uat_usecase.execute(state))

        usecase = AuditorUseCase(None, None, LLMReviewer(), e2b_tools=self.e2b_tools, github_read_tools=self.github_read_tools)
