...
from src.utils import TraceIdCallbackHandler, current_cycle_id, current_trace_id


@pytest.mark.asyncio
async def test_trace_id_callback_handler() -> None:
    ...
    config = SimpleNamespace(run_id=new_run_id, configurable={"cycle_id": "99"})
...
@pytest.mark.asyncio
async def test_state_optimization_truncation() -> None:
    ...
    with patch("src.services.workflow.GraphBuilder"):
        ...
        state = {
            "cycle_id": "01",
            "session": {"messages": [{"role": "user", "content": "msg"}] * 20},
        }
...
@pytest.mark.asyncio
async def test_rca_sanitization() -> None:
...
@pytest.mark.asyncio
async def test_workflow_background_tasks() -> None:
...
@pytest.mark.asyncio
async def test_resilient_logging() -> None:
