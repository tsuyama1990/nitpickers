...
def test_coder_graph_happy_path() -> None:
    ...
    def mock_self_critic(state: CycleState) -> dict:
        return {}

    nodes_mock.self_critic_node = mock_self_critic

    def mock_refactor(state: CycleState) -> dict:
        return {
            "committee": state.committee.model_copy(update={"is_refactoring": True})
        }

    nodes_mock.refactor_node = mock_refactor

    def mock_final_critic(state: CycleState) -> dict:
        return {"status": FlowStatus.COMPLETED}
...
def test_coder_graph_rejection_loop() -> None:  # noqa: C901
    ...
    def mock_auditor(state: CycleState) -> dict:
        ...
        return {
            "audit": state.audit.model_copy(update={"audit_result": audit_res}),
            "committee": state.committee.model_copy(update={"audit_attempt_count": state.committee.audit_attempt_count + 1})
        }
    ...
    def mock_committee_manager(state: CycleState) -> dict:
        return {}
