...
@pytest.fixture
def cycle_nodes(mock_jules_client):
    sandbox = AsyncMock(spec=SandboxRunner)
    return CycleNodes(sandbox_runner=sandbox, jules_client=mock_jules_client)

@pytest.mark.asyncio
async def test_architect_critic_node_rejected(cycle_nodes, mock_jules_client):
    state = CycleState(
        cycle_id="test-cycle",
    ...
    with patch("ac_cdd_core.graph_nodes.settings") as mock_settings:
        ...
        with patch("ac_cdd_core.graph_nodes.get_model") as mock_model:
...
@pytest.mark.asyncio
async def test_architect_critic_node_approved(cycle_nodes, mock_jules_client):
    state = CycleState(
        cycle_id="test-cycle",
        jules_session_name="test-session",
        critic_feedback=[],
