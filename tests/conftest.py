...
UAT_SCENARIO_PATTERN = re.compile(
...
class MarkdownTestItem(pytest.Item):
    """
    A Pytest item responsible for safely executing a Docs-as-Tests code payload.
    It writes the payload to a temporary file and uses `runpy.run_path` to evaluate it
    within a clean namespace, ensuring accurate stack traces without `exec()` isolation bleed.
    """
...
class MarkdownTestFile(pytest.File):
    """
    A Pytest file collector that parses Markdown specifications (`ALL_SPEC.md`, `README.md`)
    and yields `MarkdownTestItem` instances for every UAT scenario defined within.
    """

    def collect(self) -> Iterable[MarkdownTestItem]:
        content = Path(self.path).read_text(encoding="utf-8")

        matches = UAT_SCENARIO_PATTERN.finditer(content)
...
def pytest_collect_file(file_path: Any, parent: Any) -> pytest.File | None:
    """Pytest hook for collecting test files natively parsing markdown docs."""
