...
async def test_workflow_initial_pr_triggers_self_critic() -> None:
    ...
    mock_jules.wait_for_completion = AsyncMock(return_value={"status": "success", "pr_url": "http://pr/initial"})
    ...
    with patch.object(usecase, "_run_jules_session", AsyncMock(return_value=("sessions/123", {"status": "success", "pr_url": "http://pr/1"}))):
...
async def test_session_reuse_ready_for_audit() -> None:
    ...
    mock_jules.continue_session = AsyncMock(return_value={"status": "success", "pr_url": "http://pr/1"})
...
async def test_session_reuse_ready_for_final_critic() -> None:
    ...
    mock_jules = MagicMock()
    mock_jules.get_session_state = AsyncMock(return_value="COMPLETED")
    mock_jules.continue_session = AsyncMock(return_value={"status": "success", "pr_url": "http://pr/2"})
