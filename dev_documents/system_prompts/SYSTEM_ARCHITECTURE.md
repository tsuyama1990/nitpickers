...
### Network Resilience & Retry Strategy Rule
The system mandates absolute resilience against transient network failures and high-latency LLM responses. All outbound HTTP requests to external APIs (e.g., Google/Jules, OpenRouter, E2B) must be wrapped in robust asynchronous retry logic (e.g., using `tenacity` with exponential backoff) specifically trapping `httpx.ReadTimeout`, `httpx.ConnectTimeout`, and `httpcore.ReadTimeout` exceptions. The underlying `httpx.AsyncClient` configurations must utilize explicitly defined, extended timeout parameters (e.g., `JULES_REQUEST_TIMEOUT=120.0`) to prevent premature orchestration failure during complex cognitive tasks like the `self_critic_evaluator`. Under no circumstances should a simple API timeout crash the LangGraph state machine.

### Integration Circuit Breaker Rule
To prevent infinite merging loops during complex 3-Way Diff resolutions, Phase 3 (Integration Phase) must implement an explicit `integration_attempt_count` circuit breaker. If the Master Integrator agent fails to produce a conflict-free, structurally sound codebase after a defined maximum number of attempts (e.g., `NITPICK_MAX_INTEGRATION_RETRIES`), the graph must gracefully abort the merge, persist the failed state for human review, and halt the pipeline.
