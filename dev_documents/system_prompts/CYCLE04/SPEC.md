...
1. **E2B Service Wrapper:** In `src/services/e2b_executor.py`, use `e2b_code_interpreter.Sandbox` to implement the required interface. It must securely whitelist testing shell commands limiting vulnerability surface areas.
2. **TDD Validation Node:** In `src/graph_nodes.py`, the `sandbox_evaluate_node` replaces the `uat_evaluate_node` logic right after the `coder_session_node` before `auditor_node`. It will:
   - Instantiate `E2BExecutorService`.
   - Handle directory sync logic asynchronously via `anyio` bindings.
   - Run secure pytest invocations matching config mapping `test_cmd`.
   - Track evaluating state mapping to strict explicit Literal `red` or `green` testing workflows.
   - Store `E2BExecutionResult` cleanly inside `CycleState.sandbox_artifacts`.
   - Return `FlowStatus.UAT_FAILED` or `FlowStatus.READY_FOR_AUDIT` (if green).
3. **Traceback Feedback:** If `UAT_FAILED`, the node concatenates the raw `stdout` and `stderr` routing back to `coder_session`.
4. **Sandbox Protection Restrictions:** Restrict string interpolation manual shell mappings (`shlex`) isolating boundaries inside `SandboxRunner`. Explicitly require strict validation of the `E2B_API_KEY` mapped directly against `os.getenv` and `Settings`. Retries are governed strictly by configured exponential backoffs avoiding synchronous infinite blocking.
