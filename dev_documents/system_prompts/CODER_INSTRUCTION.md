...
- **CONFIGURATION**:
    ...
    - **DO** append or add new dependencies/settings to `pyproject.toml` if necessary for the feature. (Note: A final audit and cleanup of unused libraries will be performed during the project's **Final Refactoring** phase, so prioritize functionality and safety now.)
...
- **UAT Verification (`tests/uat/`)**:
  - Ensure the `tests/uat/` directory exists (create it if not). Create **Marimo** (`.py`) scripts corresponding to `dev_documents/system_prompts/CYCLE{{cycle_id}}/UAT.md`. Use descriptive filenames (e.g., `tests/uat/verify_cycle_{{cycle_id}}_domain_logic.py`).
