...
1.  **State Extension**: `CycleState` in `src/ac_cdd_core/state.py` will be extended with a new field `tdd_phase: str`, initialized to `red`.
2.  **Enum Updates**: `src/ac_cdd_core/enums.py` will include new flow statuses representing the transitions, such as `TDD_RED_SUCCESS` (meaning the test failed correctly) and `TDD_RED_FAILURE` (meaning the test erroneously passed).
...
3.  **Modify Coder Session Node**: In `src/ac_cdd_core/graph_nodes.py`, modify `coder_session_node`. When the node is invoked, it must check the `tdd_phase`. If `red`, it uses `TDD_RED_INSTRUCTION.md`. If `green`, it uses `TDD_GREEN_INSTRUCTION.md`.
...
5.  **Graph Update**: Update `_create_coder_graph` in `src/ac_cdd_core/graph.py` to correctly wire these new conditional edges.
