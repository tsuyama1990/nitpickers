...
**TASKS:**
1.  **Create Single Marimo File**: Ensure the `tutorials/` directory exists (create it if not). Then, generate a **SINGLE** Python file in `tutorials/` containing ALL test scenarios from `USER_TEST_SCENARIO.md`. Choose a **descriptive and professional filename** based on the project's domain and the current development phase (e.g., `tutorials/FePt_MgO_interface_energy.py`). Avoid generic names like `UAT_AND_TUTORIAL.py`.
    -   You MUST use **Marimo** format (pure Python, reactive).
    -   Start with `import marimo`.
    -   Use `app = marimo.App()` and define cells with `@app.cell`.
2.  **Verify**: Ensure the file runs successfully using `marimo run tutorials/<your_filename>.py`.
...
**Deliverables:**
- `tutorials/*.py` (Verified, Executable)
