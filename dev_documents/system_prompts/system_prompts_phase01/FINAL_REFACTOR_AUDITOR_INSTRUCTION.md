...
### 4. Zero Hardcoding & Dependency Hygiene
- [ ] All configuration is in `config.py` or `.env`.
- [ ] No magic numbers or hardcoded paths.
- [ ] **Dependency Hygiene**: `pyproject.toml` is clean and contains only the necessary dependencies and dev-dependencies for the final system. No unused libraries are lingering.
