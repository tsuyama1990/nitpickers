...
1.  **Connection Pooling and Lifecycle Management:** The `McpClientManager` must be defined as an asynchronous singleton dependency, utilizing the `async with` context manager pattern (`__aenter__` and `__aexit__`). It must maintain persistent, healthy connections to the underlying Node.js Stdio process across LangGraph ticks to avoid latency penalties. Importantly, the `__aexit__` hook must explicitly terminate the subprocess to prevent zombie processes (`npx`) consuming host memory if the main Python process crashes.
...
3.  **Strict Error Boundaries (Pydantic Mapping):** The manager must enforce strict timeout configurations. Raw protocol errors (`mcp.ClientError` or `mcp.ServerError`) must be intercepted and wrapped into a dedicated Pydantic model, e.g., `class ToolExecutionError(BaseModel): message: str, tool_name: str, code: int`. This guarantees deterministic schema validation before errors are fed back into the LangGraph state, preventing the LLM from hallucinating fixes for internal infrastructure failures.
4.  **Configuration Schema Validation:** The `McpClientManager` initialization parameters (e.g., specific MCP server versions, connection timeouts, paths to the `npx` executable) must be validated strictly via a dedicated `McpServerConfig(BaseModel)` object, ensuring configuration stability.
...
2.  **Create McpClientManager:** Implement `src/services/mcp_client_manager.py`. Define the `McpServerConfig(BaseModel)` to validate environment variables (`E2B_API_KEY`) and initialization arguments. Implement the connection logic using `StdioServerParameters` targeting `npx -y @e2b/mcp-server`. Wrap the lifecycle in an asynchronous context manager (`async with`) to guarantee subprocess cleanup on exit.
