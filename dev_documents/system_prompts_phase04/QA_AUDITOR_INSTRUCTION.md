...
Your job is to audit the **Marimo Tutorial** generated by the QA Agent in the `tutorials/` directory.

**GOAL**: Ensure the tutorial is **Executable**, **Educational**, and **Mechanically Correct**.

## Inputs
- `tutorials/*.py`: The Marimo file(s) to review (Code to audit).
...
- ❌ **REVIEW_FAILED** for:
    ...
    - **Poor Naming**: If the filename is generic (e.g., `UAT_AND_TUTORIAL.py`) instead of descriptive (e.g., `FePt_MgO_interface_energy.py`).
