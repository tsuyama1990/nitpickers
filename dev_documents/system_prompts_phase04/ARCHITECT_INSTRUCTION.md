...
- **Tutorial Plan**
  - You must specify that a **SINGLE** Marimo Text/Python file will be created in `tutorials/`. The filename should be **descriptive and professional** (e.g., `tutorials/FePt_MgO_interface_energy.py`). Avoid generic names like `UAT_AND_TUTORIAL.py`.
...
- **ONLY OVERRIDE** the default linter tool settings if strictly needed for project-specific requirements.
- The default configuration is initialized during the project setup (`ac-cdd init`) and is optimized for AI-generated code quality.
- **Cleanup Strategy**: Focus on *adding* necessary libraries during initial development and refinement. A final dependency audit and cleanup will be performed during the **Final Architectural Refactoring & Stabilization** phase at the very end of the project to ensure no unwanted libraries linger.

**Default Linter Configuration (Reference):**
